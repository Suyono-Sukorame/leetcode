...
var basicCalculatorIV = function (expression, evalvars, evalints) {
  let tokens = toTokens(expression).map((x) => (/^\d+$/.test(x) ? Number.parseInt(x) : x)),
  ...
  function nextUnit() {
    ...
    if (tokens[i] == "(") {
      i++;
      return nextExp();
    } else if (Number.isInteger(tokens[i])) rtn = new Exp([new Unit(tokens[i], [])]);
    else if (vals.has(tokens[i])) rtn = new Exp([new Unit(vals.get(tokens[i]), [])]);
